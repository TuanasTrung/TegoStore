...
const handleRefreshToken = async () => {
  try {
    ...
    if (!isRememberMe) {
      ...
      window.location.href = PATH_AUTH.login + '?redirectPath=' + window.location.pathname;
      return;
    }

    const refreshToken = localStorage.getItem("refreshToken");
    const { data: { token: accessToken, refreshToken: newRefreshtoken } = {} } = await _postApi(
      API_REFRESH_TOKEN,
      { refreshToken },
    );
...
const setRegisterInfo = (info) => {
  if (!info) {
    sessionStorage.removeItem('__register_info');
    return;
  }
  sessionStorage.setItem('__register_info', JSON.stringify(info));
}

const getRegisterInfo = () => {
  const info = sessionStorage.getItem('__register_info');
  if (!info) {
    return null;
  }
